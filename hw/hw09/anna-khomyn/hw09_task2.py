...
while run:
    ...
    if keys[pygame.K_RIGHT] and COORD_X+DELTA_STEP + WIDTH_RECTANGLE < pygame.display.get_window_size()[0]:
        COORD_X = COORD_X+DELTA_STEP
    if keys[pygame.K_UP] and COORD_Y-DELTA_STEP > 0:
        COORD_Y = COORD_Y-DELTA_STEP
    if keys[pygame.K_DOWN] and COORD_Y+DELTA_STEP + HEIGHT_RECTANGLE < pygame.display.get_window_size()[1]:
        COORD_Y = COORD_Y+DELTA_STEP
